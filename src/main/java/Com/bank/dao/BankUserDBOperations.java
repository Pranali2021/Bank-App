...
public class BankUserDBOperations {
	...
	public ResultSet balanceCheck(long accid)
 	{
 		...
 		try
 		{
 		PreparedStatement pr=conn.prepareStatement("select * from bnakuser where userId=?");
 		pr.setLong(1, accid);
 		rs=pr.executeQuery();
	...
	public void fundTransfer(long senderId, long receiverId, double tamount) throws SQLException
	{
		ResultSet rs=null;
		try
		{
		...
		{
			System.out.println("Insufficiant balance for fund transfer!!!");
		}
		...
		Random r=new Random();
		long tid=r.nextInt(99999);//10000
